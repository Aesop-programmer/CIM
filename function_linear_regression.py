...
class LinearPruner(BasePruningFunc):
    ...
    def prune_in_channels(self, layer: nn.Module, idxs: Sequence[int]) -> nn.Module:
        ...
        # reweight
        if (os.environ["reuse"] == 'True'):
            counter = int(os.environ["counter"])
            if(os.environ["disk"] == "True"):
                ac_dataset = vit_ac_dataset.vit_activation_dataloader(layer.get_buffer('file_name_count').item())
                input_ac = DataLoader(ac_dataset,batch_size = 1,num_workers=8)
                
                
                input_buffer = torch.tensor([])
                for k, (batch) in enumerate(input_ac):
                    input_buffer = torch.cat([input_buffer,batch[0]])
                    if int(batch[0][0].size()[0]) > 768 and k >= 3:
                        break
            else:
                input_buffer = layer.get_buffer('input_buffer')
            
            layer_tra = torch.transpose(layer.weight.data,0,1)
            
            train_x = input_buffer[:,keep_idxs]
            
            y = list(idxs)
           
            y.sort()
            
            train_y = input_buffer[:,y]
            
            reg = LinearRegression(fit_intercept=False,n_jobs=-1,copy_X=False).fit(train_x, train_y)
            ...
            for i in range(len(reg.coef_[0])):
                for j in range(len(reg.coef_)):
                    layer_tra[keep_idxs[i]] = layer_tra[keep_idxs[i]] + torch.tensor(reg.coef_[j][i])*layer_tra[y[j]]
            counter += len(reg.coef_)
            os.environ["counter"] = str(counter)
            layer.weight.data = torch.transpose(layer_tra,0,1)
        # threshold = torch.tensor(float(os.environ["threshold"]),dtype=torch.float32)
        # counter = int(os.environ["counter"]) 
        # input_average = layer.get_buffer('input_average')
        # similar_score = layer.get_buffer('similar_table')
        # layer_tra = torch.transpose(layer.weight.data,0,1)
        # for idx in idxs:
        #     most_similar_idx = -1
        #     most_similar_score = torch.tensor(0.0,dtype=torch.float32)
        #     for keep_idx in keep_idxs:
        #         if keep_idx < idx:
        #             if torch.abs(most_similar_score) < torch.abs(similar_score[keep_idx][idx]):
        #                 most_similar_score = similar_score[keep_idx][idx]
        #                 most_similar_idx = keep_idx
        #         else:
        #             if torch.abs(most_similar_score) < torch.abs(similar_score[idx][keep_idx]):
        #                 most_similar_score = similar_score[idx][keep_idx]
        #                 most_similar_idx = keep_idx
        #     if most_similar_idx == -1:
        #         raise "reweight error"
        #     if torch.abs(most_similar_score) > torch.abs(threshold):
        #         if most_similar_score > 0:
        #             layer_tra[most_similar_idx] = layer_tra[most_similar_idx]*(1+(input_average[idx]*torch.norm(layer_tra[idx]))/(input_average[most_similar_idx]*torch.norm(layer_tra[most_similar_idx])))
        #         else:
        #             layer_tra[most_similar_idx] = layer_tra[most_similar_idx]*(1-(input_average[idx]*torch.norm(layer_tra[idx]))/(input_average[most_similar_idx]*torch.norm(layer_tra[most_similar_idx])))
        #         counter += 1
        #         os.environ["counter"] = str(counter)
