...
def forward_pre_hook(self, inputs):
    ...
    if isinstance(imp, (tp.importance.SimilarImportance)):
        input = inputs[0]
        ...
        # self.register_buffer('input_average',input_average,persistent=False)
        self.N = N
    ...
    for i in range(args.iter):
        ...
        for m in model.modules():
            if isinstance(m,nn.Linear):
                ...
                hooks.append(hook)
        model.to("cuda")
        logger = TensorBoardLogger("~/nas/homes/aesop/pruning_once_similar_v3_plus", name= args.model+args.pruning_type+"threshold"+str(args.thr)+"/step:"+str(i))
    ...
    for i in range(args.iter):
        ...
        for m in model.modules():
            if isinstance(m,nn.Linear):
                ...
                hooks.append(hook)
        model.to("cuda")
        logger = TensorBoardLogger("~/nas/homes/aesop/pruning_iter_similar_v3_plus:"+f'{args.iter}'+"_acc", name= args.model+args.pruning_type+"threshold"+str(args.thr)+"/step:"+str(i))
        trainer = Trainer(callbacks=[EarlyStopping(monitor='validation_loss')],logger=logger)
        print(i)
        if isinstance(imp, (tp.importance.GroupTaylorImportance, tp.importance.GroupHessianImportance,tp.importance.SimilarImportance)):
            calculate_importance()
