...
parser.add_argument('--disk',help='resue',action='store_true')
...
def forward_pre_hook(self, inputs):
    ...
    if args.reuse:
        ...
        N = N + torch.tensor(1.)
        if args.disk:
            # input_buffer = torch.load(str("/data/tmp/")+str(self.file_name_count)+".pt",mmap=True)

            # input_buffer = torch.cat([input_buffer,input.clone().detach().cpu()])
            # torch.save(input_buffer,str("/data/tmp/")+str(self.file_name_count)+".pt")
            pass
        else:
...
import random
import numpy as np
os.environ["file_name_count"] = "0"
os.environ["disk"] = str(args.disk)
def init_hook_input(model):
    ...
    for m in model.modules():
        if isinstance(m,nn.Linear):
            ...
            if args.disk:
                # file_name_count = int(os.environ["file_name_count"])
                # m.register_buffer("file_name_count",torch.tensor(file_name_count))
                # torch.save(torch.tensor([]),str("/data/tmp/")+str(m.file_name_count)+".pt")
                # file_name_count +=1
                # os.environ["file_name_count"] = str(file_name_count)
                file_name_count = int(os.environ["file_name_count"])
                m.register_buffer("file_name_count",torch.tensor(file_name_count))
                # os.makedirs(str("/data/tmp/")+str(m.file_name_count), mode=0o777)
                file_name_count +=1
                os.environ["file_name_count"] = str(file_name_count)
            else:
...
if args.iter == 1:
    for i in range(args.iter):
        ...
        logger = TensorBoardLogger("~/nas/homes/aesop/pruning_once_reuse_v5_disk", name= args.model+args.pruning_type+str(args.reuse)+"/step:"+str(i))
    ...
    for i in range(args.iter):
        ...
        logger = TensorBoardLogger("~/nas/homes/aesop/pruning_iter_reuse_v5_disk_"+f'{args.iter}'+"_acc", name= args.model+args.pruning_type+str(args.reuse)+"/step:"+str(i))
        trainer = Trainer(callbacks=[EarlyStopping(monitor='validation_loss')],logger=logger)
        print(i)
        if isinstance(imp, (tp.importance.GroupTaylorImportance, tp.importance.GroupHessianImportance,tp.importance.SimilarImportance)):
            calculate_importance()
